...
par_combo <- function(param_set_local) {
  ...
  for (i in 1:500) {
    ...
    mu02 <- specmutual::peregrine_sim(total_time = 5,
                                      replicates = 1,
                                      mutualism_pars = mutualism_pars,
                                      verbose = TRUE)

    # And save it
    path <- paste0("~/specmutual/result/mu02", param_set_local, "_", i, ".rds")
    saveRDS(mu02, file = path)

  }
}

mu02s <- par_combo(param_set_local = param_set)
