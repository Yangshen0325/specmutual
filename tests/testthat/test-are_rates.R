...
test_that("immigration and anagenesis only happen to species from mainland", {
  ...
  expect_equal(length(immig_rate$immig_p),
               NROW(mutualism_pars$M0))
  expect_equal(length(immig_rate$immig_a),
               NCOL(mutualism_pars$M0))
  expect_equal(length(ana_rate$ana_p),
               NROW(mutualism_pars$M0))
  expect_equal(length(ana_rate$ana_a),
               NCOL(mutualism_pars$M0))
})
...
expect_equal(length(ext_rate$ext_p),
             NROW(Mt))
expect_equal(length(ext_rate$ext_a),
             NCOL(Mt))
expect_equal(length(clado_rate$clado_p),
             NROW(Mt))
expect_equal(length(clado_rate$clado_a),
             NCOL(Mt))
})
...
test_that("cospeciation, gain and loss rates for each pair of species", {
  ...
  expect_equal(prod(dim(cospec_rate)),
               prod(dim(Mt)))
  expect_equal(prod(dim(gain_rate)),
               prod(dim(Mt)))
  expect_equal(prod(dim(loss_rate)),
               prod(dim(Mt)))
})
