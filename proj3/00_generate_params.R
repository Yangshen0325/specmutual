###############################################################################
# proj3/00_generate_params.R
#
# Generate parameter combinations using Latin Hypercube Sampling (LHS) for
# a broad parameter-space exploration of the specmutual island biogeography
# model, following the approach of Simon et al. (2023).
#
# The manuscript explored only 2 levels (high/low) for 4 intrinsic rates and
# 3 levels for 4 mutualism-related parameters (total 48 unique combos).
# Here we sample 500 parameter combinations from continuous ranges that
# encompass and extend the original values, enabling random-forest analysis
# of simulation output.
#
# Run this script ONCE before submitting the cluster array job:
#   Rscript proj3/00_generate_params.R
#
# Output: proj3/param_table.csv
###############################################################################
...
# --- Settings ----------------------------------------------------------------
n_combos <- 500   # number of parameter combinations
...
cat("\nNext step: submit the cluster job with  sbatch proj3/submit_proj3.sh\n")
