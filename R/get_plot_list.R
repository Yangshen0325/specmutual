...
get_plot_list <- function(total_time,
  ...
  for (x in 1:rep) {
    ...
    nC_all <- island_total[[x]][[1]]$stt_all[, "nCp"] +
      island_total[[x]][[1]]$stt_all[, "nCa"]
    complete_arr_all[, , x] <- cbind(island_total[[x]][[1]]$stt_all[, 'Time'],
                                 nI_all,
                                 nA_all,
                                 nC_all,
                                 sum_endemics_all,
    ...
    total_p <- island_total[[x]][[1]]$stt_all[, "nIp"] +
      island_total[[x]][[1]]$stt_all[, "nAp"] +
      island_total[[x]][[1]]$stt_all[, "nCp"]
    complete_arr_p[, , x] <- cbind(island_total[[x]][[1]]$stt_all[, c(1:4)],
                                 sum_endemics_p,
                                 total_p)
    # Animal
    ...
    total_a <- island_total[[x]][[1]]$stt_all[, "nIa"] +
      island_total[[x]][[1]]$stt_all[, "nAa"] +
      island_total[[x]][[1]]$stt_all[, "nCa"]
    complete_arr_a[, , x] <- cbind(island_total[[x]][[1]]$stt_all[, c("Time",
                                   "nIa", "nAa", "nCa")],
                                   sum_endemics_a,
                                   total_a)
  }
